...
The prompts, the tuned numbers, and the expected behaviour of the direction
algorithm are not Swift constants any more. They live in [`kernel/`](kernel/),
and **that folder is the single source of truth** — the Swift values are derived
...
This exists because the same capability is going to other places — a web page, a
Telegram bot — and Swift does not run on Vercel. What can be shared is small and
mostly data, so it is shared as data. The folder stays in this repository rather
than moving to one of its own: the app and the npm package are two consumers of
one folder, in one commit, at one version.

### How a change here reaches both consumers

Edit the JSON. Nothing else. Both copies are generated, never hand-maintained.

| Consumer | Generated by | When |
|---|---|---|
| The macOS app | `Tools/generate-kernel.swift` → `Sources/FarsiTalkWrite/KernelDefaults.generated.swift` | every `make` |
| [`@mazarix/voice-kernel`](packages/web) | `packages/web/scripts/sync-kernel.mjs` → typed TypeScript + a copy of the JSON | every build, test and `npm pack` |
...
`bidi-cases.json` is deliberately run by **both** implementations. A case that
passes in one and fails in the other is the entire reason it is a shared file
rather than two sets of tests that drift apart.

What stays out of the kernel: anything a browser or a Telegram bot could not
use. Key codes, tap windows, HUD timings, insertion behaviour and permissions
are macOS-specific and remain in Swift.

### The npm package

[`packages/web`](packages/web) publishes as **`@mazarix/voice-kernel`** — the
kernel data plus a TypeScript port of `BidiText`, for the web page and the
Telegram bot.

It is **MIT licensed**, deliberately unlike the app. See the note in the Licence
section below.
...
### The kernel is MIT, not GPL

[`kernel/`](kernel/) and the npm package in [`packages/web`](packages/web) are
**MIT licensed** instead — see [kernel/LICENSE](kernel/LICENSE) and
[packages/web/LICENSE](packages/web/LICENSE).

GPL-3 is copyleft, which would mean any site or bot that installed
`@mazarix/voice-kernel` had to be GPL-3 and source-available itself. The shared
prompts and constants are meant to be usable from anywhere, so they carry the
more permissive licence. The application around them stays GPL-3.
